...
(function () {
  ...
  // Baustand (Nalpi-Export -> uploads/tables/baustand.json): erfasste
  // vormontierte PKs/Modulträger verschwinden automatisch von den Karten,
  // sobald ihr Tisch den Status "montiert" hat. (Wunsch Victor 23.07.2026)
  var baustand = null;
  fetch('uploads/tables/baustand.json', { cache:'no-store' })
    .then(function(r){ return r.ok ? r.json() : null; })
    .then(function(j){ if (j && j.tische) { baustand = j.tische; notify(); } })
    .catch(function(){});
  function montiert(r){
    if (!baustand || !r || !r.tischId) return false;
    var b = baustand[String(r.tischId).trim()];
    if (!b) return false;
    if (r.art === 'primaer') return true;            // PK steht (Status pk oder mt)
    if (r.art === 'traeger') return b.s === 'mt';    // Modulträger montiert
    return false;
  }
  ...
  function loadAll(cb){
    fbList().then(function(remote){
      ...
      items = items.filter(function(r){ return !montiert(r); });   // montiert = weg von der Karte
